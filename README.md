...
|TypeScript|実務で半年程度使用して基礎を身に付けた|
...
#### 発揮したバリュー
Nuxt.js(Vue) + Play Framework(Scala)が未経験だったが、積極的なアウトプットを意識して学習することにより短時間でキャッチアップをした。
...
プロジェクト中に困ったこととして、Nuxt.js, Play Frameworkのビルドとデプロイを数年前に使われていた社内ネットワークのJenkinsでしようとしたが、対象サーバーのDockerのバージョンが`17.04`だったためDockerfileのマルチステージビルドに失敗する事象があった。
これを解消するために対象サーバーのコピーを社内のインフラチームに作成してもらいDockerのバージョンを`17.05`以上のマルチステージビルドができるバージョンにあげた。上記問題の解消はできたが、JenkinsのコンテナからホストのDockerコマンドを叩くにはTCPの暗号化の再設定が必要という問題が新たに発生、解消するには社内のインフラチームに対応してもらう必要があったが、インフラチームのタスク的に直近で作業ができない状況だった。これを解消するためにサービス向けのGCPに構築されているJenkinsでビルドとイメージのプッシュを行い、社内ネットワークのJenkinsでデプロイ作業をすることで解消した。
